...
images =  glob.glob("/docker_data_dir/demo_input/format_6/*.jpg", recursive=False)

# initiate setup
t = weahtr.template(
  images = images,
  template = "/docker_data_dir/demo_input/format_6.jpg",
  config = "/docker_data_dir/demo_input/format_config.yml"
  )

# match all templates, write homography datat to file
t.match(method = "features", force = False)
